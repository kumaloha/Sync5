...
## ⚑⚑ **倍率 v4(2026-08-28 用户拍板:「就是按照实测(考虑时间)的概率做倍率」)**。
##
## v3 用的 `P(≥k|b)` 是**先验层的旧口径**「发 8 张(手 5 + 缓存 3)→ 8 选 5」——
## 而 `Phrase._scoring_cards()` 默认计分池只有**手牌 5 张**,缓存要逐张换进手,
## 每次 `τ_swap`=1.44s(真人实测)。旧口径的高估**随牌型难度单调递增**
## (对子 1.4× · 顺子 10.6× · 葫芦 15.3×)—— **不是平移, 是把倍率排序拧歪**。
##
## v4 口径 = `tools/vol.gd` 的**手牌 5 张 + 缓存 1 次交换(真人实测)+ 弃 ≤2 张**,
## 公式仍是 v3 的 `(1/P)^0.75` 归一到对子 ×2。新 P(b=2):
## 对子 .900 · 两对 .448 · 三条 .146 · **顺子 .0948**(旧口径 .333)· 同花 .0869 · 葫芦 .0484。
## ⚠ 顺子的新 P 与**真人实测 8%**(按持有 Target 分组, tools/tau.py)吻合到 1.19 倍,
## 旧口径差 4.2 倍 —— 这是换口径的验收锚。
##
## ⚠⚠ **顶端三档(四条/同花顺/皇家)换了函数, 不是换了数据**(2026-08-28 第二轮):
## 它们的 P **精确算得出来**(全枚举 + 闭式自检, b=0 时与 624/36/4 逐位吻合):
## 四条 **1/654** · 同花顺 **1/4,061** · 皇家 **1/40,609**(5 张口径, 弃 ≤2 追型)。
## 采样在这一档**结构上无效** —— 皇家 8000 次采样的期望命中是 **0.0123 次**,
## 要让它误差 <10% 需要 **6500 万次**。
##
## ⚑ 但精确的 P 反而暴露了**公式本身的问题**:`(1/P)^0.75` 在 P→0 时发散,
## 代进去给出 **×239 / ×941 / ×5292** —— 荒谬。**幂函数压不住 4 个数量级的概率跨度。**
## ⇒ 顶端改用**对数外推** `a + b·ln(1/P)`(两点锚定:对子 ×2、葫芦 ×18),它不发散:
## **37 / 47 / 60**。⚠ 这不是"更准",是**换了 f 的形状** —— 而 f 的形状是设计判断,
## 不是算出来的。同一套对数用在中低档会把两对 3→6、三条 7→12(策略地形大改),
## 所以**中低档仍用幂函数**(已过 sim,漂移可解释),只在顶端换 —— 那里幂函数本来就崩了。
## 顶端合计只占 **0.43%** 的拍, 换它对通关率的影响在噪声内。
##
