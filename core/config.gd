...
## 不奖励挂机。
## ⚑⚑ **门槛 = warning_offset(倒数 3·2·1 亮起的那一刻), t_run 锁等式**(2026-09-05 用户:
## 「没有规定提前结束的时间是不对的」)。旧值 4.5 在这里写过「是真难度」, 其实是**死档**:
## 真人 Tape 触发率 2.7%(数据在 run.json `_comment_early_finish`), 四张时机卡在真人手里是空白卡。
## 规则现在读作「倒数亮起前不再动手」—— 线就是屏幕上那个倒数, 不另画标记;
...
## 激励视频换金币的**两级上限**(2026-09-08;规格 docs/superpowers/specs/2026-09-08-ads-design.md §2)。
## ⚑ `AD_COINS`(一次给多少)2026-09-09 删除 —— 那个数搬到了赞助碟上, 走 `Economy.ad_coins()` 读。
