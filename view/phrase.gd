...
func _open_draft() -> void:
	state = St.DRAFT
	# ⚑⚑ **进店事件**(2026-09-03)——「每进一次商店」类成长卡(淘碟/打碟)靠它。
	# 起因:它们原本挂在 `on_reroll` 上, 而刷新实测 **0.013 次/局(≈75 局一次)**
	# ⇒ 计数器几乎不涨, 两张卡在机械意义上是白卡。进店每 3 拍一次、一局 7 次、必然发生。
	# ⚠⚠ 与 bot 侧 `tools/bot.gd::_draft` 开头那一发**必须成对** —— 漏一侧就是
	#   「规则在游戏里、不在模型里」的第八次(前七次的清单见 LESSONS 三)。
	# ⚠ 发在最前面:此刻还没有任何购买发生, 与 bot 侧同刻。
