...
const UNREACHABLE := {
	# ~~wrecker~~ 2026-08-27 已删:动作粒度修复落地(beat_discards 改**动作次数**预算 +
	# discard_batch 单批张数上限, bot 凑张数跨区缓存先行)后, kit 实测
	# **+1223 ±86(z=+14.2, 量级 31.7%, 触发 15%)** —— 正是声明里预告的那条反查路。
	# 表当前为空;结构留着, 下一张「触发条件在 bot 动作空间里做不出来」的卡照旧往里写。
}
...
func _judge(label: String, a: Array, b: Array, base: float, extra: String,
	...
	# ⚑⚑ **自称零基线的证物必须自证**(2026-09-03)。
	#
	# `want_up` 这一族货架证物被选中的**全部理由**就是「基准恒 0 / ≈0, 混杂无处藏身」
	# (见 SHOP_WITNESS 注)。可它的注释写着「无联票时物理不可能, 基准≈0」, 而实际读数
	# 是 **1.92/局** —— 三天没人看那一列。病根是 bot 每店多买一张(「5 选 1」在游戏里
	# 生效、在模型里没有), 但**照出它的是这条声明与读数打架**, 不是有人去查代码。
	# ⇒ 判据不写死阈值, 用**自校准**的:零基线的意思就是「基准 ≪ 效应」。
	#   基准 1.92 vs 效应 0.6 ⇒ 当场红(修前);基准 0.13 vs 效应 2.4 ⇒ 过(修后)。
	# ⚠ 这条 `parity.py` 抓不到 —— 它查「两侧有没有实现」, 而这里两侧都实现了,
	#   差的是**检查的时机**。静态尺量不到时序, 得让读数自己互相打架。
	# ⚠⚠ **只查自己声明了零基线的那些**(2026-09-04 修:首版扫遍所有 `want_up`,
	# 当场误报 11 条 —— 「累计金币」基准 11.37、「商店花费」基准 37.65、
	# 「含规则牌店率」基准 0.56, **它们从来没声称过基线是零**, bot 一直在赚钱花钱)。
	# ⇒ 判据取自证物**自己的声明**(`extra` 里那句「基准≈0 / 基准恒0」)——
	#   声明了才查, 没声明的本来就允许有基线。
	# ⚑ 这是我自己造的「爱喊狼来了的尺」:首版一次红 11 条, 全是假的,
	#   而一把误报的尺比没有尺更糟 —— 它会让人开始无视整列输出。
	var claims_zero := extra.find("基准≈0") >= 0 or extra.find("基准恒0") >= 0
	if want_up and claims_zero and absf(base) > 0.25 * absf(p["d"]) and absf(base) > 0.5:
		_fail.append("%s: **零基线证物的基线不是零** —— 基准 %.2f vs 效应 %.2f。"
			% [label, base, p["d"]]
			+ "这条证物被选中的理由就是「基准≈0」, 而它现在不成立 ⇒ 要么模型多做了"
			+ "本不该做的事(查两侧规则是否同刻), 要么这条证物该换。**别改卡。**")
	var verdict := "✓ 量到了"
	var why := ""
	if wrong_way:
		why = "方向反了(要的是上升)"
...
## ⚑ **消耗牌的 kit 支持:12 张全部量到**(2026-08-30 晚)。
## ⚠⚠ 此前这里写着「接了但 score 通路量不到、`pidx` 恒为 0」—— **病根不在 kit**,
## 是 `RunLoop` 的「达标即收工」在 `o.targets` 为空时判据恒真 ⇒ 一局只打 4 拍,
## 而 bot 的烧牌启发式要 `pidx >= 4`。修完 `opener` 从 **+0.0** 变 **+930.9(z=+25.83)**。
## ⚠ 那三天里 kit 的抬头一直印着「不死局**打满 24 拍**」—— **打印出来的声明不是断言**。
## ⚠ 牌堆手术卡(超级百搭/修剪)的数字要打折看:「每拍补满」对它们是**复利**
## (24 次注入 4 张万能), 不是玩家用法;定价仍以 `ccf.gd` 的单次 Δ 为准。
##
