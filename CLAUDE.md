...
SYNC5_TEST_DOMAINS=t_a,t_b godot --headless --path . --script res://tests/runner.gd   # 只跑几个测试域(分钟级;改完一批先跑相关域)
...
⚠⚠ **耗时标注会过期(已经过期过)** —— `gate.sh` 与单脸门第一步就跑全量单测, 单测一变慢它们全跟着变。
⇒ **别信文档里的秒数,跑之前先按当前单测时长估**(数字在 STATUS.md)。
...
- **候选牌机制作废**。弃牌重抽制:选中手牌/缓存的牌 → 弃牌 → **立刻原位补牌**。
  **⚑⚑ 弃牌收费 1◆/张(2026-08-26 经济 v2,用户拍板 —— 推翻 08-06「弃牌免费」,
  ...
  开局 8◆(数在 `economy.json`)· 每拍按牌型发钱(`kind_coins`,A 案)· 8 秒与金币双闸门。
  规定与健康带在 [`docs/design/levels.md`](docs/design/levels.md) 经济 v2。
  金币的出口:**弃牌 + 买牌(平价 3◆)** —— **只有两个**。
  (~~洗牌~~ 2026-08-26 随超级百搭加、**2026-09-01 用户拍板整块退役**:
  「万能牌只需要在弃牌的时候有几率洗出来就行,不用专门一个洗牌按钮」。
  ⚑ 判据是**同一件事不许有第二套机制**:`Deck.draw()` 抽干了本来就会 `_reshuffle_discard()`,
  万能牌本来就在弃牌补牌的路径上循环出来,付费重掷是重复造了一条。**别再提议加回**。
...
- **万能牌唯一来源 = 超级百搭**(2026-08-26 用户拍板**取代**百搭,wildcard 已删,
  ...
  之后**靠弃牌补牌自然循环出来**(牌堆抽干自动洗回弃牌堆);特例价 4◆(「超级卡就4金币」)。
  ⚠ ~~付费洗牌钓卡~~ 2026-09-01 退役,见上条。
...
- 结算公式(2026-08-05 三次修订,用户拍板 chips×mult):
  ...
  **牌型倍率是乘法链第一环**——稀有牌型与小丑牌相乘而不是被替代。
  ...
  而目标分逐段递增 ⇒ 它们**跟着膨胀**, 改基族则不;「前期是神」也没兑现(读数在 numbers.md §3.3)。
...
- **draft 商店已拍板并实装**(2026-08):每个 Section 结算后进商店;首张 Target 免费三选一,Support
  ...
  **首张 Target 免费三选一保留**(那是开局引导,唯一的特例)。
  ...
  让 bot 自己换掉就没了。
  收入主干 = **牌型金币 + Section 通关工资**(工资的数在 `economy.json`;4 段结构下工资点只有 4 个)。
...
- **⚑⚑ 消耗牌(2026-08-30 用户开轴,规格 = [`docs/design/consumables.md`](docs/design/consumables.md))**:
  ...
  **分界只有一条:用完之后这张卡还有没有意义** —— 一次性 ⇒ 消耗牌,持续 ⇒ 小丑牌。
  ...
  ⚑⚑ **全部自动触发(2026-09-01 用户拍板,推翻同年 08-30 的「实时可点」)**:
  「现在玩起来有点怪,比如那个塞 4 张万能卡,还要自己点一下才生效,完全不用点」。
  ⇒ **「点」这个动作整个没了**,栏位随之退役。触发时机写在数据的 `fire` 上:
  `"buy"` = 买下即触发(13 张 `action`:改牌堆或改这次商店的,**没有「哪一拍」可选**)·
  `"next"` = 下一拍(快闪)· `1..6` = 遇到的第一个第 N 拍(**开场① · 副歌④ · 彩头⑥**)。
  ⚑ 四张时机卡因此收成**一条规则**而不是四条特例,而且那个数**刻在碟面上** —— 规则自解释。
  ⚠ 代价认下:`boost` 那 4 张原来「烧在哪一拍」是真决策,自动化拿掉了它;换来的是
  13 张 `action` 不再有确认对话框式的空点击。**判据是「点下去有没有选择发生」。**
  与小丑槽**互不占用**(那是**装备位**)。
  ⚑⚑ **但货架位是共用的(2026-08-31 用户拍板「其实是 5 选 1」)**:每店 **3 小丑 + 2 消耗 = 5 选 1**,
  消耗牌**没有专属成交名额**, 它和小丑牌抢同一次购买。~~商店每店上架 1 张且不进三选一~~ 已推翻。
  ⚑⚑ **唱片位 = 待播队列(2026-09-01)**:排队中的消耗牌以**碟**的样子并排在唱片原来的位置,
  标签上刻着自己的拍号,**左→右就是播放顺序**;空着时就是一张转着的唱片。
  用户原话:「光碟的位置还原成光碟」+「币变成碟就可以了,最多应该是 3 个」。
  ⚑ **「币」与「碟」是同一样东西** —— 商店里你买的是一张碟,买完它排进歌单,到点它播;
  形状本身在做分类:**卡牌留在槽里,碟是花掉的**。恒用**卡牌的红** `SUIT_RED`,空碟是青绿。
  ⚠ 显示 3 张、第 4 张叠成「+1」,**不设硬上限** —— 上限 4 是算出来的(商店每 3 拍一次,
  一张卡最长等 5 拍 ⇒ 跨得过一次商店跨不过两次;联票能在一店买 2),一条几乎不触发的规则
  不值得让玩家去理解。
...
信息区 → 「♪ 小丑牌 ♪」标签 → 小丑牌 ×4 → **盲注卡 + 音浪 + 唱片**(同一层,左中右)
→ 均衡器竖条带 → 手牌区 5 张(走圈轨道 + 节拍菱形;主角小人已删,轨道保留)→ 缓存区 3 张 + 理牌/弃牌

**局内盲注 = 一张牌**(`Widgets.BlindCard`):摆在音浪层左边、和右边**唱片**对称的一张竖牌。
⚑ 唱片(`VinylDeck`)**2026-09-01 回位**:08-31 它的三个职责(提前收工键 / 牌堆剩余计数 /
转盘装饰)被逐条取消、位置让给了消耗品栏;09-01 消耗牌改成全部自动触发, 栏位这个概念没了,
位置还给光碟, 并接上**新职责 = 待播队列**(见上「消耗牌」那条)。
⚠ **金色角标(牌堆剩余)与金环(提前收工)不还原** —— 那两个机制真的退役了,
画回来等于把删掉的东西又画上。**牌堆剩余张数仍然不显示**(用户拍板)。
