...
> ⚑ **段分独立 + 达标即收工(2026-08-27 用户拍板:先「还是每个盲注独立,不改」,
> 随即改口「选 A」)**:段分照旧独立清零(与 Balatro 一致:一个 ante 三盲注 1×/1.5×/2×
> 各自门槛),**但补上原作真正的出口 —— cash out**:打到目标分即可立刻收工,
> 剩余拍数换金币。原来的痛点是**超额分纯浪费**(我们 6 拍必须打满,原作可以提前结束),
> 补上之后「继续榨分 vs 落袋为安」变成一个真选择,也给经济 v2 加了一条收入通道。
> ⚠ 全局累加(总分到了就行)**明确不做** —— 前期一次爆发就能躺平后三段,
> 「4 段全是 BOSS 墙」的拍板与整套难度曲线会失效。

## ⚑⚑ 经济 v2(2026-08-26 用户压缩解空间 —— 设计稿,数字待建模)

> 用户:「关卡、经济,我给你的约束比较少,你在一个很大的解空间求解……我来做一些规定,
> 压缩解空间,你在剩余空间里找解(需调研)。」四条规定(**方向定死,数值可优化**):
>
> 1. **每张小丑牌 ≈3◆**,特例需说明(现行 4/6/9/11 压平;稀有度只留出现权重轴)。
...
**连带账(每条都要落)**:
- ⚠ **弃牌影子价 κ 回归求解器/bot**(S10 时代因免费被删,「少一个要扫的参数」——现在加回,
  机制级改动,单测/pair/门全套);`beat_discards` 手速预算与金币预算并存,谁紧谁生效;
- S9「金币边际近零」结论过期;金币单调性哨兵将重新敏感(好事);
- 洗牌 3◆ vs 逐弃 5 张 5◆:洗牌成为「整手重掷折扣价」,定价张力健康,保留;
- 段工资 `section_clear_reward` 去留待建模(初判:留小额兜底或删,看每拍收入方差);
- **牌型金币表(2026-08-26 纸面建模,先验层 P(≥k) 口径 —— 与牌型倍率同一把尺)**:
  用户三锚(高牌 1 / 对子 2 / 皇家 10)与达成难度 −log₂P(≥k) 几乎同构,唯一例外是
  **对子:8 选 5 下 92.9% 达成,信息量近零** —— 给 2 是纯保底工资。两案:
  · **A 案 ✅ 用户已拍(2026-08-26)**:1/2/2/3/4/5/6/8/9/10(Two Pair 压到与对子同格)——
    期望 ≈2.1◆/拍 ≈ 50◆/局;真人口径支出(弃 1.5 张/拍 ×24 = 36◆ + 买卡 12◆)≈ 48◆,
    **局末余额 ≈ 10±,微松**,试玩可校;
  · B 案(备选未启用):High/Pair 同为 1 —— 仅当 A 真人实测过松时再议。
  ⚠ bot 世界恒比真人松(bot 弃 0.6 张/拍 vs 真人 1.5-3)—— **健康带按真人口径标**,
  sim 报表只看相对变化;弃牌收费后两边的弃牌分布都会下移,⑥ 落地后重量。
- 落地清单:economy.json(starting_coins/discard_cost/joker_prices/牌型金币表挪入)·
  core(κ)· bot/solver · 测试(弃牌收费契约复活)· CLAUDE.md 反转标注 · 全量门(新基线)。

### 怎么调(2026-08-26,用户:「数值按感觉拍的,帮我想想怎么调」)

**先定判据(好的经济长什么样),再动数字** —— 四条健康带,全部可测:

1. **紧度**:局末余额中位数落在**个位数**(S9 时代 34.7◆ = 钱不是决策,是背景音);
   「想买买不起」发生率在 20-40% 的商店访问(0% = 钱无意义,>60% = 商店是橱窗);
2. **弃牌要疼但不瘫**:弃牌张数分布相对免费时代**下移但不塌零**
   (塌零 = 玩家吓得不敢弃 = 8 秒闸门失去玩点;不动 = 1◆ 无感);
3. **无硬死锁天然成立**(出牌永远免费,最差打烂牌),但盯**软破产率**:
   「某拍 0◆ 且想弃弃不起」的拍占比给个上限(先拍 15%,真人数据校);
...
牌型→金币用**同一把尺**:`coin(kind) ≈ round(a − b·log₂ p(kind))` 圆整
(用户拍的 高牌1/对子2/皇家10 本来就近似 log 形状 —— 频率 46% 的对子给 2、
频率≈0 的皇家给 10)。先验层 p(kind) 零数据可精算,新牌型当场出数。

**六步走**(numbers.md SOP 的经济化):
① 先验层解析算期望收入/拍(Σ p·coin)与支出(bot 弃牌分布 × 1◆),纸面对齐健康带 1;
② sim 全人群跑收支报表(局末余额分布/破产率/买卡数/弃牌分布)对齐 2-4;
③ 不达标只压/抬**表的斜率 b**(锚 a 定在 高牌=1),不逐格手调;
④ 段工资:若 ② 显示前两段购买力断粮 → 留 +1◆ 保底,否则删;
⑤ probbook 补行(设计/仪器两列),真人列等用户 Tape;
⑥ 真人试玩终审(弃牌手感这种东西仪器量不了「疼」,只有你能拍)。
