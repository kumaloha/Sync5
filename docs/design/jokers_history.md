...
## 2026-08-16 · 加分族诊断、A 案推导与牌型倍率展开(2026-08-21 从 TODO 搬来,原文未改)

> 结论早已落地(加分族 13 张走 `bonus_target_pct`、两对 ×3 已恢复),这里保留的是**推导与实测数**。
> 仍开着的只有「顶端三档牌型倍率(四条/同花顺/皇家)要先定压缩指数」—— 归用户,挂在 TODO 🔴 表。
