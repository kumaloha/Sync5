...
生成:2026-08-28 16:34 · 真人样本:19 局 · bot 来源:**未提供(缺 bot 列)**
真人列两态:**无R** = 未持有任何规则牌(近道/四指/双色调/百搭)的拍,**有R** = 持有的拍。bot 列未拆态(仪器债)。
**`p_基线` = 换分母那一列(2026-08-28)**:分母是**全部合格拍**而不是「持有该卡的拍」,因为行为/牌面类条件成不成立与有没有装那张卡无关 —— 样本因此从 3~6 拍变成 250 拍。每一条判定都由 `fired` **逐拍自证**(持有该卡的拍上必须与游戏记录一致),一致率不足 100% 的**不出值**,只报「判定不同源」。
⚠ 读数时记得两条偏差:① **行为适应** —— 装了谢幕的人会更爱压秒,所以基线是「不特意配合」的**下界**,持有态那列才是含配合的真实收益,两列并排才完整;② 只覆盖能从已记事实直接判定的卡,其余留 `—`。

| id | 名 | 稀有 | 通道数额 | p̂ 设计 | p_bot (n拍) | p_人·无R (n拍) | p_人·有R (n拍) | **p_基线 (n拍)** | 诊断 |
|---|---|---|---|---|---|---|---|---|---|
| encore | 回响 | common | bonus_target_pct 0.577 | 0.35(重复上一拍:需刻意保型,且被禁回族脸打断) | — | 33% (15) | — | — | ⚠人样本薄 |
| finale | 尾声 | common | bonus_target_pct 0.24 | 0.65(最后2秒有操作:节奏玩家多数拍会压秒) | — | 62% (21) | — | **45%** (250)<br>近期 57% (130) | ⚠人样本薄 ⚠版本漂移 |
| turnover | 周转 | common | per discard | 0.75(弃过牌:免费弃牌下多数拍会弃) | — | 73% (15) | — | **52%** (250) | ⚠人样本薄 |
| tipjar | 小费罐 | common | coins 2 | 0.35(整拍零弃:与弃牌自由互斥,少数拍) | — | 50% (6) | — | **48%** (250) | ⚠人样本薄 |
| chord | 和弦 | common | bonus 60.0 | 0.70(建成态:缓存三同花建好后可持续;建设期2-4拍) | — | — | — | — | · |
...
| vip | 贵宾 | uncommon | additive_face_value 20 | — | — | — | — | **68%** (250) | 基线未自证(没持有过) |
| glowstick | 荧光棒 | uncommon | bonus_pct … | — | — | 100% (6) | — | — | ⚠人样本薄 |
...
| reprise | 复读 | uncommon | bonus_pct 1.4 | 0.35(同回响) | — | — | — | — | · |
| fullcast | 全员 | common | bonus_target_pct 0.769 | 0.12(顺/同花/葫芦成手:等价于打出大牌型) | — | 8% (12) | — | **11%** (250) | ⚠人样本薄 |
| superfan | 铁粉 | uncommon | cap 0.15 | 0.90(持币≥2◆ 几乎恒真) | — | — | — | — | · |
| opener | 开场 | uncommon | bonus_pct 1.5 | 0.17(每段首拍:1/6 结构概率) | — | — | — | — | · |
| rainbow | 彩虹 | common | bonus_target_pct 0.433 | 0.28(成牌四花色:C(4,4)分布+换牌可凑,偏彩票) | — | 38% (24) | — | **28%** (250) | ⚠人样本薄 |
| nopair | 清流 | common | bonus_target_pct 0.457 | 0.40(五张无对:高牌/顺/同花态,可刻意保持) | — | 0% (3) | — | **17%** (250) | ⚠人样本薄 p̂≫基线 |
| rehearsal | 排练 | common | bonus 200.0 | 0.08(缓存三连号:同上) | — | 0% (6) | — | — | ⚠人样本薄 |
| bassclef | 低音谱 | uncommon | additive_low_value 15 | — | — | 83% (18) | — | **74%** (250) | ⚠人样本薄 |
| warmtone | 暖色 | common | card_filter red | — | — | 33% (3) | — | **98%** (250) | ⚠人样本薄 ⚑基线≈无条件(>95%),按地板定价 |
| cooltone | 冷色 | common | card_filter black | — | — | 100% (6) | — | **95%** (250) | ⚠人样本薄 |
| undertone | 低声部 | common | card_filter rank_lte_5 | — | — | 67% (21) | — | **74%** (250) | ⚠人样本薄 |
| duo | 对唱 | common | additive 10 | — | — | — | — | **83%** (250) | 基线未自证(没持有过) |
| duet | 二重唱 | uncommon | bonus_pct 0.25 | — | — | 100% (3) | — | **83%** (250) | ⚠人样本薄 |
| triad | 三和弦 | common | additive 25 | — | — | — | — | **18%** (250) | 基线未自证(没持有过) |
| triplebill | 三重 | uncommon | bonus_pct 0.6 | — | — | — | — | **18%** (250) | 基线未自证(没持有过) |
...
| stilllife | 静物 | common | bonus_target_pct 0.26 | — | — | — | — | **34%** (250) | 基线未自证(没持有过) |
| segue | 串场 | common | per swapped_scoring | — | — | 67% (6) | — | — | ⚠人样本薄 |
| stageexit | 让位 | common | per face_discard | — | — | 25% (12) | — | — | ⚠人样本薄 |
| royalty | 分成 | uncommon | coins_factor 2 | — | — | — | — | — | · |
| skint | 穷开心 | rare | mult_add 0.3 | — | — | — | — | — | · |
| curtain | 谢幕 | uncommon | bonus_pct 0.6 | — | — | 67% (9) | — | **21%** (250) | ⚠人样本薄 |
| stopwatch | 秒表 | uncommon | per second_left | — | — | — | — | — | · |
| earlyout | 早弃 | common | bonus_target_pct 0.192 | — | — | — | — | **27%** (250) | 基线未自证(没持有过) |
...
| deejay | 打碟 | rare | per counter:n | — | — | 0% (2) | — | — | ⚠人样本薄 |
| goldenvoice | 金嗓 | rare | per coins:6 | — | — | — | — | — | · |
| hush | 静场 | rare | mult_add 0.4 | — | — | — | — | **48%** (250) | 基线未自证(没持有过) |
| harmony | 和声 | rare | mult_add 0.12 | — | — | — | — | — | · |
