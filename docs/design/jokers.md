...
each hand is now (chips, mult) Balatro-style (table in docs/design/cards.md), the
...
## 历次批次的经过 → [`jokers_history.md`](jokers_history.md)

流派批两波 · 引擎波次子波 1/2/3 · 仪器债第一批 · 杂项裁决 · 观察点结案 ——
这些**按日期排的批次流水账**(含各自的实测读数)2026-08-15 拆去
[`jokers_history.md`](jokers_history.md),**没有删**。

⚠ 判据(用户 2026-08-15):**文档只留调研 + 最新结果,中间过程归 CHANGELOG**。
本篇因此只保留**当前**的卡表、定价口径与设计原则。

## ⚑ 转型(rebrand)与「反复换旗」路线(2026-08-29 用户拍板「换旗该有最高胜率, 灵活打法我鼓励的」)

**改动**:加 `shelf.target_guaranteed`(货架保证出现 Target),稀有度保持 `uncommon`
(它是这条路线的**唯一入口**, 出现率优先于「带两个效果该升档」—— 一度升 rare, 读数反而变差)。
卡面 `Targets always offered; each change +40%`。

**同批的机制修正(比数值重要)**:**换旗不再占用购买名额**(`view/phrase.gd` + `tools/bot.gd` 同改)。
原状态下 `buy_limit=1` 而换旗算一次成交 ⇒ **想买转型就不能换旗, 想换旗就买不到转型** ——
奖励某个动作的卡与那个动作抢同一个资源, **结构上自我否定**。
实测证据:修前**持有转型的局换旗 0.65 次 < 未持有的 0.94 次**(装了增益卡反而更少)。
修后:换旗路线 18.8% → 23.9%(污染表口径)· 转型持有局数 75 → 117 · lift +4% → +18% ·
**其余五条路线一个数没动**(只解锁, 没削人)。kit 单卡门:计数器终值 +1.7 次/局(z=44.8)· 总分 +508(36.5%)。

⚠ **仍挂着**:持有转型的局换旗次数仍略低于未持有(0.78 vs 0.99), 残余挤压未定位。
