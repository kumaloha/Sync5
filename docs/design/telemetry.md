...
- **`settle` 是最重的一条**。`fired` 记 joker **id 而不是 popup 文案** —— 只有 id 能和
  `report.gd` 的 `trigger_n` 直接对齐,文案改一个字就对不上。
  (`@character` 口径随主角系统 2026-08-24 删除退役。)
...
⚑ **口径仍是「只记事实, 不记特征」**:记「在第几拍烧了哪张」, **不记**「为什么烧」
或「烧得值不值」—— 后者是会迭代的判断, G/D 那边自己从事实推。

⚠ **`why` 是必要的**, 不能省(2026-09-01 由 `at_ctx` 改名并换了口径):消耗牌改成
**全部自动触发**之后, 「在哪个语境点的」这个区分消失了 —— 玩家不再点。取而代之要记的是
**它为什么在这一刻打**:买下即触发 / 到了拍号 / 被帕奇欧复制出来。
⚑ 这仍然是**事实**不是特征:`buy` 与 `due` 是两条不同的因果路径, 重放要靠它区分
「这一拍的加成从哪来」。而「烧得值不值」照旧不记。
