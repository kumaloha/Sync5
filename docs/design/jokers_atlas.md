...
> ⚠⚠ **2026-08-14 更正:上面这段手算与实现不符,实际稀 2.3 倍。**
> 本段假设「先掷档(5%)再档内均分」,而 `view/shop.gd::_weighted_pick` 是
> **每张卡各带权重、一起不放回抽** —— 张数不均时两者完全不同。
> 先验层解析读数(`SYNC5_PRIOR_MODE=shelf`,零采样):
> **rare 实际档占比 3.9%(不是 5%),一局期望只出 0.82 张 —— 平均连一张稀有卡都见不到**;
> 特定稀有卡一局 **4.8%**(本段手算给的是 11.0%)。
> ⚠ common/uncommon 的实际占比恰好≈名义权重,**纯属两档张数相同(22 vs 22)的巧合**,
> 改任何一档张数三档比例就全变,**而没有任何测试锁着它**。
> ⚑ **本段的结论方向仍然成立**(扩池 ⇒ 稀释 ⇒ 滑向抽签),**但所有绝对数字要按解析值重读**。
> 完整表与推导 = [`prior.md`](prior.md) §5.7。
...
| 主角联动卡 | (主角系统 2026-08-24 已删,此类永久出局) |
| 主动技能卡 | 全被动是拍板(8 秒的注意力归时钟) |
| 算术条件卡(「手牌点数和 ≥N」) | 压力语言是操作不是心算 |

### 2.19 标准自查记录(2026-08-10,用户点名「回忆标准再说」之后补)

对照 docs/design/jokers.md 的 A1–A5 / B1–B4 / C1–C6 / D1–D3 逐张过了一遍,结果:
...
## 6. 出图清单(60 张,id 已冻结 —— GPT 出图与引擎波次并行)

**文件名约定:`assets/jokers/joker_<id>.png`,即插即用**(素材分工既有管线)。
id 从此冻结,出图不用等实装。画面提示只描述主体,风格走既有霓虹舞台语言。
...
| stair | 阶梯 | 通往聚光灯的霓虹台阶 |
| mono | 单色 | 单色灯光淹没的舞台 |
| triplet | 三连音 | 三个重叠的音符/三人和声 |
| lonewolf | 独狼 | 独自站在麦克风前的狼 |
| kaleido | 万花筒 | 万花筒里不断变形的牌面 |
| shredder | 速弹 | 吉他速弹、指尖残影 |
| wrecker | 拆迁 | 铁球砸穿牌墙(实装暂缓,先出图) |

### Support · 已实装 15 + 现役 18
...
| tipjar | 小费罐 | 塞满硬币的玻璃罐 |
| chord | 和弦 | 三根同色琴弦 |
| neonsign | 灯牌 | 一块纯粹的霓虹招牌 |
| vinyl | 黑胶 | 唱针下的黑胶纹路 |
| chorus | 副歌 | 全场合唱的高潮 |
| interest | 利息 | 生出小硬币的硬币 |
| momentum | 惯性 | 越滚越快的节拍飞轮 |
| vip | 贵宾 | 贵宾席上的 J/Q/K |
| glowstick | 荧光棒 | 渐暗的荧光棒 |
| shortcut | 近道 | 跳过一级的台阶 |
| fourfingers | 四指 | 四指手套 |
| twotone | 双色调 | 半红半黑的舞台灯 |
| bassline | 贝斯线 | 低音贝斯的重复律动 |
| mirror | 镜面 | 映出另一张牌的镜子 |
| wildcard | 百搭 | 大小王探出牌堆 |
| variation | 变奏 | 同一旋律的两种写法 |
| reprise | 复读 | 重复的乐句记号 |
| fullcast | 全员 | 五人全员谢幕 |
| superfan | 铁粉 | 攥着钱包尖叫的粉丝 |
| opener | 开场 | 拉开的大幕第一束光 |
| popup | 快闪 | 快闪演出的临时海报 |
| rainbow | 彩虹 | 四色灯同时打向舞台 |
| nopair | 清流 | 五张互不相认的牌 |
| backup | 伴唱 | 三个人头牌在和声 |
| rehearsal | 排练 | 排成一串的候场牌 |
| trio | 三重唱 | 三张同点数并肩而立 |
| bassclef | 低音谱 | 放大的低音谱号托起小牌 |
| warmtone | 暖色 | 红桃方块沐浴暖光 |
| cooltone | 冷色 | 黑桃梅花浸在冷光 |
| undertone | 低声部 | 小点数牌合唱低音 |

### Support · 待引擎波次 19(id 已冻结,可先出图)

| id | 卡 | 画面提示 |
|---|---|---|
| curtain | 谢幕 | 幕布落下前的最后一躬 |
| stopwatch | 秒表 | 停在剩余秒数上的秒表 |
| freeze | 定格 | 定格的舞姿投影到下一拍 |
| earlyout | 早弃 | 开场前扔出的牌 |
| crescendo | 渐强 | 渐强记号推高的音浪 |
| segue | 串场 | 从候场滑进聚光灯的牌 |
| stilllife | 静物 | 纹丝不动的缓存三张 |
| declutter | 断舍离 | 整手牌一把撒出 |
| stageexit | 让位 | 人头牌鞠躬退场 |
| doggybag | 打包 | 打包带走的溢出分数 |
| royalty | 分成 | 唱片分成的双倍支票 |
| digger | 淘碟 | 在唱片堆里淘碟的手 |
| collector | 收藏家 | 排满墙的收藏卡 |
| doublebill | 联票 | 一票两场的联票票根 |
| sponsor | 赞助 | 印着折扣的赞助横幅 |
| skint | 穷开心 | 口袋空空却笑得最响 |
| rebrand | 转型 | 撕掉旧海报露出新旗 |
| trim | 修剪 | 剪掉 2、3 的修枝剪 |
| xray | 透牌 | 透视下一张补牌的荧光屏 |
...
- **拆迁 × 配给:用户结案(2026-08-10)——「这是策略问题,不是设计问题」**。
  盲注开局全公开(设计边界 1),看着配给墙还举拆迁旗是玩家的路线错误,正是 routing 技能的考题
  (同款先例:独狼预见禁回墙就该在商店窗口换旗)。**预防性调参撤销**,交叉臂数据将来只当观测不当门。
  留下的规格细节仍然有效:「三次弃牌」按张数计不按按键计(免得一口气墙无意双杀)。
  ⚠ **实施状态(2026-08-10 首波)**:拆迁 kit 实测**触发 0%**——规则 bot 不会为一面旗改弃牌策略
  (kit 判据②「接上了但条件没发生」)。按「不许为门绿改卡」处置:**卡暂撤出 json,
  等 bot 侧加弃牌流策略块(独狼 vow 先例)再入**。设计不变,欠的是仪器。
  ⚠ **仪器债第二笔(同日二波)**:三重唱分差 +376(z=2.36 @ solver n=40)——量级够(5.3%)
  但在默认预算下稳定不显著(配对种子确定性,非侥幸),而 `_arm_n` 只封顶不加量。
  同样撤出 json 待仪器/数值任一变化;伴唱(+631, z=4.06)与排练(+560, z=3.96)
  在修正后的分差判据下双线过门,已入池。**仪器债清单:wrecker、trio。**
