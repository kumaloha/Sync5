...
> **状态:已实装(2026-09-09, 计划 5 任务 + 2b 审查修补;`sponsor_free_text` 键最终并入 `free_text`;玻璃壳中性色, 金色只来自光)。**
...
- 认下:赞助碟每店可见,免费 3◆ 会被多数人每次都拿 ⇒ 实际经济向 +6◆/局漂;上限锁两次,数在 JSON;`income_ad` 与真人 Tape 的拿取率盯着(分母 = `shop` 事件的 `ad` 位)。
- 认下(终审指出的顺序约束):5 选 1 下买任何一张 = 当场离店,所以「买完再看广告」不存在,玩家必须**先看后买**;带联票名额时店不关,赞助碟仍在架。
