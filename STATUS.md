...
>
> **最后更新:2026-09-05 晚**(增量见文末「增量快照」节,数字冲突以那节为准)
...
| 项 | 状态 | 命令 |
|---|---|---|
| 单元测试 | **3726 passed / 0 failed**(2026-09-09 赞助商版后, `run.sh` 带锁 08:54 起跑, 四判据全过:passed=3726 · SCRIPT ERROR=0 · ^ERROR=0 · exit 0)。⚠ 耗时 ~55 分钟 |
...
⚑ 2026-09-06 「镜像不许手抄数字」筛出四处搬进 JSON:牌型表 → `data/patterns.json` · 色板 → `data/theme.json` · `view/layout.gd` 的装配坐标 → `ui.json.stage` · `RESOLVE_FEEDBACK` → `run.json`。
`lua/data/*.lua` 是它们的生成物(`tools/luagen.py`)。
...
## 增量快照(2026-08-10 ~ 09-06,与上文冲突时以本节为准)
...
- **2026-09-05 晚 · 试玩三报闭环**(细账 CHANGELOG 09-05 晚):三张「本店」消耗牌(加急/赞助/挑高)在 5 选 1 下
  ...
  (真人 Tape 触发率 2.7% → 24%;t_run 锁等式与卡面秒数)。五域单测 1397/0 + t_lingo/t_shop 715/0;
  截图目视(商店免费态/赞助 1◆/装备槽卡面)。**唱片位视觉批**:碟从货架飞入唱片位, 到点的落进队列、即生效的转一圈溶掉。
  **设计判断批**:镜面去连击 · 穷开心上限 12→5(= 卡面)· 三重 +45% 加分 → ×1.5 乘数;清流/全员/12 张消耗牌定价不动。
  未过门(用户:能不跑就不跑);四张时机卡与这三张的 kit/price 旧读数作废。
