...
| `price.gd` | 每(脸, 轮)一个价 | ⚠⚠ **2026-09-01 实测:不再是「挂过夜」那一档** —— 19.4 小时只跑出 **7 行(全是 S1)**, 池里 52 张脸 ⇒ 按这个速度是**以周计**。原因是**截断修复**:探针从每局 4 拍恢复成 24 拍, 全线 ×6, 而 price 本来就是最贵的那个(N=150 × 每格一次完美玩家推演)。⇒ **再跑之前必须先改造**(降 N / 分片 / 换更便宜的估计量), 否则它会一直占着机器什么也交付不了。当次已 kill;`ranking.json` 继续用 rankgen 的上一版顶着 |
...
| **`evsync.py`** ⚑ | **evbook 的实测 EV → `sim.json` 的 `ev.measured`**(2026-08-30 新)。它是 bot 估值的**地板**(权重 0.5), 修的是「手写信念表系统性低估」。⚠ **手改 `ev.measured` 无效** —— 改数据的唯一路径:`cf.gd` 刷 evbook → `evsync.py` 同步 → 跑 sim。`--check` 已进 `gate.sh` 与 `t_db`(结构侧) | 秒级 |
