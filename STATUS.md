...
| 项 | 状态 | 命令 |
|---|---|---|
| 单元测试 | **3037 passed / 0 failed**(2026-08-30 消耗牌批;含 t_consumable 101 条。⚠ 耗时从 12 分涨到 ~31 分 —— 删掉 goldenvoice 的 SOLVER_BLIND 豁免后 t_draft 要真跑一次求解器推演, 这是**永久性**的。2026-08-28 试玩反馈四批 + 定价仪器批;四判据全满足)。⚑ 比 08-27 的 2664 多 **+367**,主要是两道**排版预算门**:`t_lingo` ⑤(盲注卡两行 × 中英两态)+ ⑥(小丑卡一行 × 三种真实尺寸)—— 「渲染函数放不下就丢且不报错」这一类从此锁着 | `./tools/unittest.sh`(四判据唯一一份;裸跑 = `godot --headless --path . --script res://tests/runner.gd`) |
...
| **`parity.py`** ⚑⚑ | **游戏侧 / bot 侧的规则对齐扫描**(2026-08-30 新)。「规则在游戏里、不在模型里」本项目栽过**七次**, `core/joker.gd` 早写着方法(「grep 就能数清两侧是否对齐」)—— **有尺没用**, 所以做成可执行的:数每个规则入口在 view/ 与 tools/ 的调用数, 一侧为零就红。首用即抓到帕奇欧(只接了游戏侧, sim 里是纯废卡)。已进 `gate.sh`。⚠ **加新钩子时要同时加进 ENTRIES 表** —— 那张表本身就是这条纪律 | 秒级 |
...
| **`cfg.prefer`**(sim cohort) ⚑ | **强制 bot 试用指定的一批卡**(2026-08-30 新)。用途:分辨**「内容真的弱」与「仪器看不见」** —— 一条 cohort、五分钟, 而按错误结论重做内容要几天。首用即翻案:46 张「从没进过赢局」的卡, 强制试用后通关率 42.7~45.4%, **全部高于正常路线均值** | 随 sim |
...
- **2026-08-15 ~ 17 · 教学关重做 + 升级系统 + 券 + 三处重锚**(细账在 [TODO.md](TODO.md)
  ...
  `section_targets` 由**真人试玩重锚** `[420, 1500, 3100, 5600]`(增量递增、终点不动,
