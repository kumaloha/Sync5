...
| 小丑牌覆盖门 | **69 张**(⚠ 2026-08-30 消耗牌开轴:9 张转生走、+帕奇欧);单卡 kit 直跑 ~10 秒(不含单测)。⚠ **kit 还不认识消耗牌** | `SYNC5_KIT_ID=<id> godot --headless --path . --script res://tools/kit.gd` |
...
| | 现役 |
|---|---|
| 小丑牌 | **69 张** · **消耗牌 12 张**(2026-08-30 开轴, 见 [consumables](docs/design/consumables.md))。⚠ 这一行历史上写错过多次(23/61/63/76 都出现过)—— **一律以 `data/*.json` 的计数为准, 别手抄** |
...
#### sim 全队列基线(2026-08-28,`tools/sim.gd` 1000 局/队列)
...
尺子自检 ✅(random 1.0% ≤ 5% · 最强 48.5% ≥ 3%)。经济健康带:①局末余额中位 49◆
**偏离 +40◆(带 = 个位数)** ②弃牌 0.99 张/拍、零弃 41%(**1◆ 对 bot 无感,未下移**)
③软破产 0.01% 达标 ④牌型金币收入斜率为正 达标。

⚠ **观察点**:`adaptive:triplet` 48.5% 显著高于其余队列(9~18%),而 `earlyout`
改窗口后进了它的胜利套件(×4,全队列最多)。**这一趟无法归因** —— 上一次有记录的
读数是 08-13(twin 47.3 / triplet 38.3),中间隔了经济 v2 与倍率 v3 终表。下一趟按本表比。
