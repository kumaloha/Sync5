...
func run(t) -> void:
	...
	t.eq(Joker.pool().size(), 64, "65 jokers(2026-08-29 首批转生 9 张;2026-08-30 二批转生:四张规则牌(近道/四指/黑调/红调)——它们的 `acquire.deck_rule` 把规则**烙进牌堆且没有撤销路径**, 卖掉后规则依然生效 ⇒ 按判据「用完之后这张卡还有没有意义」= 一次性)")
