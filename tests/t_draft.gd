...
##      **求解器照旧看不到它**:它一拍只弃「没被握住的那 ≤3 张」, 4 张仍在动作空间之外。
##      扩求解器的弃牌集是 S5 的活。
##   ⑤ **完美玩家打法族**(2026-08-13)—— 「与上一拍相同牌型」类的卡(回响/复读)。
##      推演用的是**完美玩家**:它每拍都追得到最优牌型, 于是牌型天然多变,
##      `same_as_prev` 在推演里一次都不成立。而 kit 用的是**规则 bot**(会重复打),
##      那边触发 22%、+1242 —— 所以卡是活的, 只有这个仪器看不见它。
##      ⚠ **它们此前"测得到"其实是噪声**:把 `beat_budget.discards` 从 2 校准到 4 之后
##      求解器更容易追到不同牌型, 那点偶然的重复就没了 —— 校准把一个假信号照了出来。
...
const SOLVER_BLIND := {
	...
	"encore": "⑤ 完美玩家追最优, 牌型每拍在变, same_as_prev 恒假(kit 规则 bot 侧触发 22%)",
	"reprise": "⑤ 同上",
