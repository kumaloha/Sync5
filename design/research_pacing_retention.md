...
> 2026-08-07。`design/gates.md` 列的四个方向的调研结果。
...
| # | 发现 | 为什么重要 |
|---|---|---|
| 1 | **手游 session 中位数 = 4分45秒,我们一局 4.9 分钟** | 参照系确认了:**我们的留存结构更像短局手游,不像 Balatro(30-90 分钟)**。design/gates.md 的预判成立 |
| 2 | **「高 D1 低 D7」的公认解读 = 缺中期深度或 meta 进度** | 我们的 meta(`design/ui_meta.md`)**整块未做**。这不是"以后再说",它直接决定 D7 |
...
> 但真正优化的量必须是**可计算的**——也就是 `design/gates.md` 那三个因子。
>
> 这条同时支持 `design/gates.md` 的做法:**先做敏感性分析,不要拟合权重**。
...
> 所以近失对我们**处在最有效的那一端**。
>
> 更重要的是:**近失被解读成「我快学会了」** —— 那和 `design/gates.md` 的
> **成长感**是同一个心理量。所以:
>
...
> `design/gates.md` 的纪律(绝对值不可信,只信相对排序)。
...
`design/gates.md` 的预判成立:**参照系应该是短局手游,不是 Balatro。**
...
我们的 meta(`design/ui_meta.md`)**整块未做**,`HomeScreen.PROFILE` 里 LV/经验/金币/宝石
...
> 这条要在 design/ui_meta.md 动工前先想清楚,别做到一半才发现和 §1 的铁律撞车。
...
`design/gates.md` 那条推论——「引入新脸的那段定得更松,下一段用同一张脸产生掌握感」。
...
| 4 | 心流当词汇不当指标;**先敏感性分析不拟合权重** | §3 心流缺可操作判据 | 支持 design/gates.md |
...
| 9 | **meta 未做 = D7 的主要风险** | §5「高 D1 低 D7 = 缺 meta」 | design/ui_meta.md 的优先级要上调 |
...
| 12 | catch-up meta 与「DDA 不可见」冲突,要做成奖励不是同情 | §5 + §1 | design/ui_meta.md 动工前的前置约束 |
...
- [More Than Meets the Eye: The Secrets of Dynamic Difficulty Adjustment (Game Developer)](https://www.gamedeveloper.com/design/more-than-meets-the-eye-the-secrets-of-dynamic-difficulty-adjustment)
...
- [The Discomfort Zone: The Hidden Potential of Valve's AI Director (Game Developer)](https://www.gamedeveloper.com/design/the-discomfort-zone-the-hidden-potential-of-valve-s-ai-director)
