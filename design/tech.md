...
> ② 数值与内容全部在 `data/*.json`,`core/db.gd` 硬校验;
...
Adding a third opcode requires the same bar as a new hook (design/jokers.md).
...
- `view/home.gd`(`HomeScreen`)= 首页,规格 `resources/home.html`。整屏自绘,
...
- meta 数值(等级/经验/双货币)是 `HomeScreen.PROFILE` 一处占位,design/ui_meta.md 未做。
...
- **`design/gates.md` 第 2 项(可加性检验)不依赖本文档**,可以并行。
...
| 验什么 | 怎么验 | 失败长什么样 |
|---|---|---|
| **schema 正确** | `core/db.gd` 硬校验 | 未知键/坏引用**直接红**,不静默 |
