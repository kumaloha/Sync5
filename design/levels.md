...
> **Re-cut 2026-08-06 (pacing pass, design/levels.md)**: shops are now **decoupled
...
count did not. Full derivation in `design/levels.md`.
...
play + 7 shops ≈ **4.9 min**, inside the 5-minute ceiling (design/levels.md).
...
`view/run_end.gd`, 1:1 from `resources/success.html` + `fail.html` (still
...
| 盲注不可跳过 | 已是铁律(design/levels.md) | 不能靠 skip 压缩时长,只能压**单拍秒数 × 拍数** |
| 12 个盲注 | 四场 × 三盲(design/levels.md) | 比原作(Balatro 8 底注 × 3 = 24)少一半,是有意为之 |
...
### 为什么商店是 7 次不是 4 次 —— 槽位是瓶颈不是容量目标
  ...
  **为什么商店是 7 次不是 4 次**:槽位(4)是**瓶颈**不是容量目标——7 商店 : 4 槽 ≈ 1.75:1,
  ...
  砍到 4 次会让折半回收/Target 换旗/成长牌三个已实装机制全部失活,推导见 `design/levels.md`。
